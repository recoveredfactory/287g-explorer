...
<main id="main-content" class="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
  ...
  <div class="mt-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <h1 class="text-2xl font-black leading-tight text-ink-900 sm:text-3xl">
          {agency.name}
        </h1>
        <p class="mt-1 text-base text-ink-700">
          ...
          {#if agency.state}
            {#if agency.agency_type || agency.city || agency.county}<span class="mx-1">·</span>{/if}<a
              ...
              class="text-ink-700 no-underline hover:underline"
            >{agency.state}</a>
          {/if}
        </p>
      </div>
  ...
  {#if agency.notes && agency.notes.length > 0}
    <aside class="mt-6 rounded-lg border-l-4 border-[#BE6079] p-4 text-sm" style="background: var(--color-paper-100); color: var(--color-ink-900);">
      <ul class="space-y-2">
  ...
  {#if agency.terminated_date}
    <div class="mt-6 rounded-lg border-l-4 border-[#BE6079] px-4 py-3" style="background: var(--color-paper-100);">
      <p class="text-sm font-bold uppercase tracking-wide" style="color: var(--color-ink-900);">{m.agency_ended_heading()}</p>
  ...
  {#snippet agreementCard(ag: Agreement)}
    ...
    <article class="rounded-xl border border-paper-200 bg-paper-100 p-5 sm:p-6">
      ...
      <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        ...
        {#if ag.pdf_url}
          <a
            ...
            target="_blank"
            rel="noreferrer"
            class="shrink-0 text-sm font-semibold text-ink-500 no-underline hover:text-ink-900 hover:underline"
      ...
      {#if ag.lea_signer_name}
        <div class="mt-4 border-t border-paper-200 pt-4">
          <p class="text-sm font-semibold text-ink-900">{m.agency_signed_by_party({ party: agency.name })}</p>
          <dl class="mt-2 grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 text-sm">
            <dt class="whitespace-nowrap text-ink-500">{m.agency_contact_name()}</dt>
            <dd class="min-w-0 font-medium text-ink-900">{ag.lea_signer_name}</dd>
          </dl>
        </div>
      {/if}
      ...
      {#if ag.ice_signer_name || ag.ice_field_office}
        <div class="mt-4 border-t border-paper-200 pt-4">
          <p class="text-sm font-semibold text-ink-900">{m.agency_signed_by_party({ party: "ICE" })}</p>
          <dl class="mt-2 grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 text-sm">
            {#if ag.ice_signer_name}
              <dt class="whitespace-nowrap text-ink-500">{m.agency_contact_name()}</dt>
              <dd class="min-w-0">
                <span class="font-medium text-ink-900">{ag.ice_signer_name}</span>
                {#if ag.ice_signer_title}<span class="block text-ink-500">{ag.ice_signer_title}</span>{/if}
              </dd>
            {/if}
            {#if ag.ice_field_office}
              <dt class="whitespace-nowrap text-ink-500">{m.agency_field_office()}</dt>
              <dd class="min-w-0 text-ink-700">{ag.ice_field_office}</dd>
            {/if}
          </dl>
        </div>
      {/if}
      ...
      {#if ag.moa_poc_name || ag.moa_poc_address || ag.moa_poc_phone || ag.moa_poc_email}
        <div class="mt-4 border-t border-paper-200 pt-4">
          <p class="text-sm font-semibold text-ink-900">{m.agency_agreements_pa_contact()}</p>
          <dl class="mt-2 grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 text-sm">
            {#if ag.moa_poc_name}
              <dt class="whitespace-nowrap text-ink-500">{m.agency_contact_name()}</dt>
              <dd class="min-w-0 font-medium text-ink-900">{ag.moa_poc_name}</dd>
            {/if}
            {#if ag.moa_poc_address}
              <dt class="whitespace-nowrap text-ink-500">{m.agency_contact_address()}</dt>
              <dd class="min-w-0 text-ink-700">{ag.moa_poc_address}</dd>
            {/if}
            {#if ag.moa_poc_phone}
              <dt class="whitespace-nowrap text-ink-500">{m.agency_contact_phone()}</dt>
              <dd class="min-w-0"><a href="tel:{ag.moa_poc_phone}" class="text-ink-700 hover:underline">{ag.moa_poc_phone}</a></dd>
            {/if}
            {#if ag.moa_poc_email}
              <dt class="whitespace-nowrap text-ink-500">{m.agency_contact_email()}</dt>
              <dd class="min-w-0"><a href="mailto:{ag.moa_poc_email}" class="break-words text-ink-700 hover:underline">{ag.moa_poc_email}</a></dd>
            {/if}
          </dl>
        </div>
      {/if}
  ...
  {#if showAgreements && agency.moa_url}
    <section class="mt-10">
      <h2 class="font-serif text-xl font-bold text-ink-900">{m.agency_moa_heading()}</h2>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        class="mt-3 inline-block rounded border border-paper-200 px-4 py-2 text-sm font-medium text-ink-700 no-underline hover:bg-paper-100"
      >
  ...
  {#if !showAgreements && singleAgreement}
    <section class="mt-10">
      ...
      <p class="mt-3 text-xs italic text-ink-500">{m.agency_pa_contact_note()}</p>
    </section>
  {/if}
  ...
  {#if showAgreements && agency.agreements}
    <section class="mt-10">
      <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <h2 class="font-serif text-xl font-bold text-ink-900">{m.agency_agreements_heading()}</h2>
        {#if coverage && coverage.modelsListed > 1}
          <p class="text-xs font-semibold uppercase tracking-wider" style="color: {isPartial ? '#BE6079' : 'var(--color-ink-500)'};">
            {m.agency_agreements_coverage({ onFile: coverage.onFile, total: coverage.modelsListed })}
          </p>
        {/if}
      </div>
      <p class="mt-1 text-sm text-ink-500">{m.agency_agreements_caption()}</p>
  ...
  <section class="mt-10">
    <h2 class="font-serif text-xl font-bold text-ink-900">{m.agency_contact_heading()}</h2>
    ...
    {:else}
      <p class="mt-3 text-sm italic text-ink-700">{m.agency_contact_none()}</p>
    {/if}
  </section>
  ...
  {#if historyRows.length > 0}
    <section class="mt-10">
      <h2 class="font-serif text-xl font-bold text-ink-900">{m.agency_history_heading()}</h2>
      <p class="mt-1 text-sm text-ink-700">{m.agency_history_intro()}</p>
      <ol class="mt-5 space-y-0 border-l-2 border-paper-200 pl-5">
        {#each [...historyRows].reverse() as event}
          ...
          <li class="relative pb-5 last:pb-0">
            ...
            {#if event.flagFirstSeen}
              <p class="mt-0.5 text-xs text-ink-500">{m.agency_first_seen()} · {dateFmt(event.detectionDate)}</p>
            {/if}
  ...
  <!-- Dive deeper -->
  <section class="mt-10">
    <h2 class="font-serif text-xl font-bold text-ink-900">{m.agency_records_heading()}</h2>
    <p class="mt-2 text-ink-700"><Gloss text={m.agency_records_intro()} {seen} /></p>

    {#if muckrock.requests.length > 0}
      <div class="mt-5">
        <p class="text-sm font-semibold text-ink-700">{m.agency_records_matched_intro()}</p>
        <ul class="mt-3 space-y-3">
          {#each muckrock.requests as req}
            <li class="overflow-hidden rounded-lg border border-paper-200 shadow-sm">
              <div class="flex items-baseline justify-between gap-4 bg-paper-100 px-4 py-2">
                ...
                <p class="text-xs text-ink-500">MuckRock #{req.foia_id}</p>
              </div>
              <div class="bg-paper-50 px-4 py-3">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  class="text-sm font-semibold leading-relaxed text-ink-900 no-underline hover:underline"
                >"{req.title}"</a>
                <p class="mt-1.5">
                  <a
                    href={req.absolute_url}
